...
Finally, \autoref{chap:conclusion} talks about the general future direction of this research and lists some of the next steps towards it.

Throughout this thesis, the reader is assumed to be knowledgeable about partial orders, limits, derivatives, Taylor and Maclaurin series, and monads. Monads will be represented with a bold upper-case sans-serif letter (e.g. $\monad{M}$), and their respective functors are represented by the same letter in regular font weight (e.g. $\functor{M}$). The set of natural numbers is represented with $\N$ and is understood to consist of all non-negative integers ($0 \in \N$)