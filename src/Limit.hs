...
class Limit a r where
    ...
    --formally, |f n - r| <= 2^(-n) implies limit f = r
    limit :: (Int -> a) -> r
    limit f = listLimit (map f [0..])
    --default limit :: (Fractional a) => (Int -> a) -> r
    --limit f = errorLimit [(f n, fromRational $ 1 % pow2 n) | n <- [0..]]
...
--Receives a predicate function to test the condition e <= 1/2^n
